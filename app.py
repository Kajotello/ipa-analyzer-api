...
async def trains(train_number, day, month, year):
    collection = db.get_collection('trains').find({'number': int(train_number), 'day': int(day), 'month': int(month), 'year': int(year)})
    my_list = []
    async for element in collection:
...
async def train_in_date(date):
    collection = db.trains.find({'year': int(date[:4]), 'month': int(date[5:7]), 'day': int(date[8:10])})
    my_list = []
...
async def travel_time_example():
    collection = db.trains.aggregate([
        ...
        {
            '$group': {'_id': '$schedule.point.name', 'avg_travel_time': {'$avg': '$schedule.travel_time'}}
        },
        {
...
async def travel_time_in_year(category, year):
    ...
    collection = db.trains.aggregate([
        ...
        {
            '$group': {'_id': {'month': '$month', 'point_position': '$schedule.point.position'},
    ...
    async for element in collection:
        ...
        for stop_info in element['schedule']:
            if stop_info['departure_time']:
                train_info['schedule'].append({'x': datetime.datetime.fromtimestamp(stop_info['departure_time']), 'y': stop_info['point']['position']})
                train_delayed_info['schedule'].append({'x': datetime.datetime.fromtimestamp(stop_info['departure_time']+ stop_info['departure_delay']*60), 'y': stop_info['point']['position']})
                insert_position += 1
            if element['direction'] == 1:
            ...
            if stop_info['arrival_time']:
                train_info['schedule'].insert(insert_position, {'x': datetime.datetime.fromtimestamp(stop_info['arrival_time']), 'y': stop_info['point']['position']})
                train_delayed_info['schedule'].insert(insert_position, {'x': datetime.datetime.fromtimestamp(stop_info['arrival_time'] + stop_info['arrival_delay']*60), 'y': stop_info['point']['position']})
                insert_position += 1
...
async def travel_time_in_year(data: QueryData):
    if data.category == 0:
        data.category = [1, 2]
    else:
    ...
    collection_year = db.trains.aggregate([
        {
            '$match': {'year': data.year, 'category': {'$in': data.category}, 'direction': {'$in': data.direction}}
        },
        {
            '$unwind': {'path': "$schedule"}
        },
        ...
        },
        {
            '$project': {
                ...
                'is_delay_gained': {
                    '$cond': [{'$gt': [ {'$subtract': ['$schedule.departure_delay', '$schedule.arrival_delay']}, 0]}, 0, 1]
                }
            }
        },
        {
            '$group': {'_id': {'point_name': '$schedule.point.name', 'point_position': '$schedule.point.position'},
                       'train_count': {'$sum': 1},
                       ...
                       'avg_stop_time': {'$avg': '$schedule.stop_time'},
                       'max_delay_gained': {'$max': {'$subtract': ['$schedule.departure_delay', '$schedule.arrival_delay']}},
                       'avg_delay_gained': {'$avg': {'$subtract': ['$schedule.departure_delay', '$schedule.arrival_delay']}},
                       'percentage_of_punctual_on_arrival': { '$avg': '$is_punctual_on_arrival'},
                       'percentage_of_punctual_on_departure': { '$avg': '$is_punctual_on_departure'},
                       'percanteg_of_train_without_gained_delay': {'$avg': '$is_delay_gained'},
                       'avg_real_stop_time': {'$avg': {'$subtract': [{'$add': ['$schedule.departure_time', {'$multiply': ['$schedule.departure_delay', 60]}]}, {'$add': ['$schedule.arrival_time', {'$multiply': ['$schedule.arrival_delay', 60]}]}]}}
    ...
    collection_month = db.trains.aggregate([
        {
            '$match': {'year': data.year, 'month': data.month, 'category': {'$in': data.category}, 'direction': {'$in': data.direction}}
        },
        {
            '$unwind': {'path': "$schedule"}
        },
    ...
    collection_day = db.trains.aggregate([
        {
            '$match': {'year': data.year, 'month': data.month, 'day': data.day, 'category': {'$in': data.category}, 'direction': {'$in': data.direction}}
        },
        {
            '$unwind': {'path': "$schedule"}
        },
...
async def stat(data: StationQuery):
    ...
    collection = db.trains.aggregate([
        ...
        {
            '$match': {'schedule.point.name': data.station_name, 'schedule.has_stop': 1}
        },
        {
            '$group': {
                '_id': {
                ...
                'arrival_delays': {
                    '$push': "$schedule.arrival_delay"
                },
                'departure_delays': {
                    '$push': "$schedule.departure_delay"
                },
                'stop_times': {
                    '$push': "$schedule.stop_time"
                },
                'delay_gained': {
                    '$push': {'$subtract': ["$schedule.departure_delay", "$schedule.arrival_delay"]}
                }
             }
        }
    ...
    async for element in collection:
        arrival_del, arr_del_bins = np.histogram(element['arrival_delays'], [-10, -5, -3, -2, -1, 0, 1, 2, 3, 4, 5, 10, 15, 20, 30, 40, 50, 60, 80, 100, 120, 300, 1000])
        departure_del, dep_del_bins = np.histogram(element['departure_delays'], [-10, -5, -3, -2, -1, 0, 1, 2, 3, 4, 5, 10, 15, 20, 30, 40, 50, 60, 80, 100, 120, 300, 1000])
        stop_times, stop_times_bins = np.histogram(element['stop_times'], [-10, -5, -3, -2, -1, 0, 1, 2, 3, 4, 5, 10, 15, 20, 30, 40, 50, 60, 80, 100, 120, 300, 1000])
